// Feed sounds: every reel loops its own synthesized track (Web Audio, no files).
(() => {
  "use strict";
  ...
  function label(tr) {
    return tr.artist ? tr.name + " - " + tr.artist : tr.name;
  }
  ...
  TRACKS.forEach((tr, i) => {
    ...
    row.innerHTML =
      ...
      "<div>" +
      '<div class="snd-title"><span class="snd-num">S' + (i + 1) + '</span><span class="snd-name">' + label(tr) + '</span><span class="snd-genre">' + tr.genre + " · " + tr.bpm + " BPM</span></div>" +
      "<p>" + tr.note + "</p>" +
      "</div>" +
  ...
  function setLive(i) {
    if (i === live) return;
    live = i;
    if (soundOn) play(TRACKS[live]);
    render();
  }

  // Switch once the scroll settles on a reel, like TikTok starting the next video.
  let settle = 0;
  feed.addEventListener("scroll", () => {
    clearTimeout(settle);
    settle = setTimeout(() => {
      setLive(Math.round(feed.scrollTop / feed.clientHeight));
    }, 90);
  });
